...
import play.modules.elasticsearch.{ElasticSearchException, JsonUtils}
...
class MappingWithFieldData(val mapping: Mapping, fieldData: FieldData) extends NestableMapping with JsonUtils {
  ...
  override def toJson: JsObject =
    mapping.toJson match {
      case JsObject(Seq((field, definition: JsObject))) => JsObject(Seq((field, definition + ("fielddata" -> fieldData.toJson))))
    }
