...
object MappingTests extends Specification with NoTimeConversions with ClientUtils {
  ...
  "index should" >> {
    ...
    "have a create method that defines mappings for the index" in new WithTempIndex {
    ...
    "type should" >> {
      ...
      "throw an exception when defining an invalid mapping" in new WithTestIndex {
        ...
        val badMapping = ObjectMapping(temporaryIndex.typeName, properties = Set(
  ...
  def createTestIndexWithMapping(implicit i: TemporaryIndex) =
    awaitResult(testIndex.create(Seq(testMapping)))
