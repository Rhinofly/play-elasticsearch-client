...
object StemmerTokenFilter {
  ...
  def normalizeJson(json: JsValue) =
    if ((json \\ "name").isEmpty)
      ...
      val JsObject(fields) = json.as[JsObject]
      val map = fields map { case(componentName, componentDefinition) =>
        val JsObject(fields) = componentDefinition.as[JsObject]

        val componentDefinitionNorm = fields map {
          case(key, value) if (key == "name") => ("language", value)
          case(key, value) => (key, value)
        }

        (componentName, new JsObject(componentDefinitionNorm))
      }
