...
object Mapping {
   ...
   */
  def jsonForMappings(mappings: Seq[Mapping]): JsObject =
    Json.obj("mappings" -> JsObject(
      mappings map {_.toJson} collect {
        case obj: JsObject => obj.fields(0)
      }
    ))
  ...
  def mappingsFromJson(mappings: JsValue): Seq[Mapping] = mappings match {
    case JsObject(values) if values.size == 1 =>
      val (indexName, indexValue: JsObject) = values.head
      indexValue.fields match {
        case Seq(("mappings", mappings)) => mappings match {
          case JsObject(fields) => (fields map TypeMapping.fromJson).toSeq
